...
public class FrontController extends HttpServlet {
    ...
    private ActionBase getAction(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            //該当するAction オブジェクトを作成(例：リクエストからパラメータ action=Employee の場合、actions.EmployeeActionオブジェクト)
            type = Class.forName(String.format("actions.%sAction", actionString));

            //ActionBase のオブジェクトに(例:actions.EmployeeActionオブジェクト→actions.ActionBaseオブジェクト)
